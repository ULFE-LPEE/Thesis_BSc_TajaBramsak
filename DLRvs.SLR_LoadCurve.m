...
for k = 1:n
    ...
    % Aproksimacija spremembe smeri vetra v stopinjah [°]
    ...
    T_f = 0.5 * (T_s + T_a); % temperatura tanke plasti zraka v neposrednem stiku s površino vodnika
    lambda_f = 2.368e-2 + 7.23e-5 * T_f - 2.763e-8 * T_f^2; % [W/K·m] toplotna prevodnost zraka  pri temperaturi T_f
    gamma = (1.293 - 1.525e-4 * y + 6.379e-9 * y^2) / (1 + 0.00367 * T_f);  % gostoto zraka 
    mu_f = (17.239 + 4.635e-2 * T_f - 2.03e-5 * T_f^2) * 1e-6;  % dinamično viskoznost zraka 
...
%% 2) IZRAČUN SLR ( izracun enak kot DLR samo da vzamem konservativne predpostavke V,T_a in I_T)
for k = 1:n
    % Konstantne spremenljivke
    V = 0.61;    % hitrost vetra
    T_a = 35;      % okoljska temperatura
    I_T = 1000;    % globalno obsevanje
    delta_deg = 60;   % smer vetra
    ...
    ro = ro_20 * ( 1 + alfa_20*(T_a - 20));
        %beta_20 * (T_a - 20)^2); % drugi kvadratni del lahko zanemarim
    A = ((pi * (D)^2) /4);
    Rdc = ro / A;
    Rac = k_ac * Rdc;
...
% Graf: tri spremenljivke iz DLR
 ...
 xlabel('Ura [h]');
 title('24-urni potek hitrosti vetra, temperature in sončnega obsevanja');
 ...
 xlabel('Ura [h]');
 ylabel('Kot \delta [°]');
 title('24-urni potek smeri vetra kota \delta');
 grid on;
 xlim([0 23]);
 ylim([0 180]);
 xticks(0:2:24);
...
% DLR tokovno zmogljivost od največje proti najmanjši
I_sorted = sort(I_DLR, 'descend');

% časa v dnevu (ker imamo 23 vrednosti, vsak predstavlja ~4.35 %)
n = length(I_sorted);
...
% Navpične črte za vizualno interpretacijo
...
% DLR moč od največje proti najmanjši
%P_sorted = sort(P_DLR, 'descend');
%n = length(P_sorted);
%percent_time = (1:n) / n * 100;

% Statična moč (konzervativno npr. P_SLR(1) ali povprečje)
%P_static = mean(P_SLR);
...
% Navpične črte za časovno interpretacijo
%xline(60, '--', 'Color', [0.3 0.3 0.8]);
%xline(80, '--', 'Color', [0.3 0.3 0.8]);
%xline(90, '--', 'Color', [0.3 0.3 0.8]);
